...
module internal PlantUml =
    ...
    let ensure rid = async {
        ...
        if archiveMatches asset then
            ...
            stage asset
            return Downloaded(version, rid)
    }

    let ensureCurrent () = async {
        let rid = currentRuntimeId () |> RuntimeId.value
